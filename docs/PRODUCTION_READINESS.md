...
| --- | --- | --- |
| Django ticketing interface | Implemented | Includes analytics, profile editing, CSV export, bulk actions, and notification readiness. Pull the update and complete the browser pilot. |
| Fake users and tickets | Implemented | Run the pilot-data command, then follow `docs/PILOT.md` on approved laptops. |
| LAN address/DHCP reservation | Tooling ready; waiting for approval | Run `lan-readiness.sh`, obtain the actual address, apply it to Ubuntu, then run `configure-approved-lan.sh`. |
